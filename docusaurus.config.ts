import {createRequire} from 'node:module';
import {themes as prismThemes} from 'prism-react-renderer';
import type {Config} from '@docusaurus/types';
import type * as Preset from '@docusaurus/preset-classic';

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

// Local, offline search index (built at compile time) — no external service,
// suitable for a self-hosted/private site (no Algolia crawler).
const require = createRequire(import.meta.url);

const config: Config = {
  title: 'automate4z',
  tagline: 'Bridge your z/OS workflows with modern DevOps practices',
  favicon: 'img/favicon.ico',

  future: {
    v4: true,
  },

  url: 'https://zarafa-dev-io.github.io',
  baseUrl: '/automate4z-docs/',

  organizationName: 'zarafa-dev-io',
  projectName: 'automate4z-docs',

  onBrokenLinks: 'throw',

  // `.md` files are parsed as CommonMark (not MDX), so the auto-generated
  // changelog — mirrored verbatim from the code repo's CHANGELOG.md, which is
  // full of `${{ … }}` and `<placeholders>` — never breaks the build. `.mdx`
  // pages keep full MDX. (Docs authored here use `.mdx`; the only `.md` file is
  // docs/changelog.md.)
  markdown: {
    format: 'detect',
  },

  // Even if you don't use internationalization, you can use this field to set
  // useful metadata like html lang. For example, if your site is Chinese, you
  // may want to replace "en" with "zh-Hans".
  i18n: {
    defaultLocale: 'fr',
    locales: ['fr', 'en'],
    localeConfigs: {
      fr: { label: 'Français' },
      en: { label: 'English' },
    },
  },

  presets: [
    [
      'classic',
      {
        docs: {
          sidebarPath: './sidebars.ts',
          editUrl: 'https://github.com/zarafa-dev-io/automate4z-docs/tree/main/',
        },
        blog: false,
        theme: {
          customCss: './src/css/custom.css',
        },
      } satisfies Preset.Options,
    ],
  ],

  themes: [
    [
      require.resolve('@easyops-cn/docusaurus-search-local'),
      {
        hashed: true,
        indexBlog: false,
        docsRouteBasePath: '/docs',
        language: ['fr', 'en'],
        highlightSearchTermsOnTargetPage: true,
      },
    ],
  ],

  themeConfig: {
    image: 'img/a4z-social-card.png',
    colorMode: {
      defaultMode: 'dark',
      respectPrefersColorScheme: true,
    },
    navbar: {
      title: 'automate4z',
      logo: {
        alt: 'automate4z',
        src: 'img/logo-a.png',
        width: 32,
        height: 32,
      },
      items: [
        {
          type: 'docSidebar',
          sidebarId: 'tutorialSidebar',
          position: 'left',
          label: 'Docs',
        },
        {
          type: 'localeDropdown',
          position: 'right',
        },
        {
          // Public entry point: the main a4z repo is private, issues live here.
          href: 'https://github.com/zarafa-dev-io/automate4z-docs/issues',
          label: 'GitHub',
          position: 'right',
        },
      ],
    },
    footer: {
      style: 'dark',
      links: [
        {
          title: 'Documentation',
          items: [
            {
              label: 'Getting Started',
              to: '/docs/intro',
            },
          ],
        },
        {
          title: 'Project',
          items: [
            {
              label: 'GitHub',
              href: 'https://github.com/zarafa-dev-io/automate4z-docs',
            },
            {
              label: 'Issues',
              href: 'https://github.com/zarafa-dev-io/automate4z-docs/issues',
            },
            {
              label: 'Licence (EULA)',
              to: '/docs/licence',
            },
          ],
        },
      ],
      copyright: `Copyright © ${new Date().getFullYear()} automate4z. Tous droits réservés — logiciel distribué sous licence EULA.`,
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,
};

export default config;
